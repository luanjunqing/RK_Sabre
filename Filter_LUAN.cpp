 ...
 * @Last Modified time: 2019-02-17 13:44:43
 */
...
void filter_sobelX(IplImage *img_Filtered, IplImage *img_Original){
    ...
    for(i=1;i<height-1;++i){
        for(j=1;j<width-1;++j){
            ...
            window[8] = (char)Data_original[(i-1)*step_gray + (j+1)]*1;
            tmp_imgData = sum(window, WINDOW_LEN);
            if(tmp_imgData > 255){
                Data_filtered[i*step_gray + j] = 255;
            }
			else if(tmp_imgData < 0){
                Data_filtered[i*step_gray + j] = 0;
            }
			else{
                Data_filtered[i*step_gray + j] = tmp_imgData;
            }
        }
    }
}
...
int main(int argc, char const *argv[]){
	...
	IplImage *Image_OUT;
	IplImage *Image_MED;
	IplImage *Image_SOX; //image with sobel X filter
	IplImage *Image_SOY; //image with sobel Y filter
	// IplImage *Image_SOM; //image with both sobels merged
	...
	uchar *Data_Med;					// pointeur des données Image_MED
	uchar *Data_Sox;					// pointeur des données Image_SOX
	uchar *Data_Soy;					// pointeur des données Image_SOY
	// uchar *Data_Som;					// pointeur des données Image_SOM
	...
	// cvNamedWindow("Image_SOM_Window", CV_WINDOW_AUTOSIZE); 	// Image_SOM

	// Positionnement des fenêtres
    cvMoveWindow("Image_IN_Window", 0,0);
    cvMoveWindow("Image_OUT_Window", 0,700);
	cvMoveWindow("Image_MED_Window", 700,0);
	cvMoveWindow("Image_SOX_Window", 700,700);
	cvMoveWindow("Image_SOY_Window", 1400,700);
	...
	// Image_SOM = cvCreateImage(cvSize(Image_IN->width,Image_IN->height),  IPL_DEPTH_8U, 1);

	int step_gray = Image_OUT -> widthStep/sizeof(uchar);

    // Boucle tant que l'utilisateur n'appuie pas sur la touche q (ou Q)
// #ifdef _PROFILING
    // unsigned char _main_n_loops=MAIN_LOOPS;
    // while(_main_n_loops--) {
// #else
